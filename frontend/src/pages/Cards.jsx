          ...
          <section className="mb-4">
            <h6 className="sidebar-heading px-2">Personal Settings</h6>
             <div className="d-flex flex-column gap-1 mt-3">
                                     ...
                                     <Link to="/cards" className="sidebar-btn-link active text-decoration-none">
                ...
                {showFilterOverlay && (
                  ...
                  >
                    ...
                    <div className="mb-3">
                      ...
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          ...
                          onChange={() => handleCheck("cardStatus", "markedComplete")} id="c0" />
                        <label className="form-check-label" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="c0">Marked as complete</label>
                      </div>
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          checked={filters.cardStatus === "notMarkedComplete"}
                          onChange={() => handleCheck("cardStatus", "notMarkedComplete")} id="c1" />
                        <label className="form-check-label" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="c1">Not marked as complete</label>
                      </div>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          checked={filters.dueDate === "noDates"} onChange={() => handleCheck("dueDate", "noDates")} id="d0" />
                        <label className="form-check-label d-flex align-items-center gap-2" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="d0">
                          <i className="bi bi-dash-circle" style={{ fontSize: "14px" }}></i> No dates
                        </label>
                      </div>
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          checked={filters.dueDate === "overdue"} onChange={() => handleCheck("dueDate", "overdue")} id="d1" />
                        <label className="form-check-label d-flex align-items-center gap-2" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="d1">
                          <i className="bi bi-exclamation-triangle" style={{ fontSize: "14px", color: "#f87168" }}></i> Overdue
                        </label>
                      </div>
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          checked={filters.dueDate === "nextSevenDays"} onChange={() => handleCheck("dueDate", "nextSevenDays")} id="d2" />
                        <label className="form-check-label d-flex align-items-center gap-2" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="d2">
                          <i className="bi bi-calendar-week" style={{ fontSize: "14px" }}></i> Next seven days
                        </label>
                      </div>
                      <div className="form-check">
                        <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                          checked={filters.dueDate === "nextMonth"} onChange={() => handleCheck("dueDate", "nextMonth")} id="d3" />
                        <label className="form-check-label d-flex align-items-center gap-2" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor="d3">
                          <i className="bi bi-calendar-month" style={{ fontSize: "14px" }}></i> Next month
                        </label>
                      </div>
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      <select className="form-select form-select-sm border-secondary text-light" style={{ backgroundColor: "#1d2125", fontSize: "0.85rem" }}>
                    ...
                    <div>
                      ...
                      {[
                        { id: "a0", value: "activeDay", label: "Active in the day" },
                        { id: "a1", value: "activeWeek", label: "Active in the week" },
                        { id: "a2", value: "activeMonth", label: "Active in the month" },
                        { id: "a3", value: "activeLast", label: "Active in the last" },
                      ].map(({ id, value, label }) => (
                        <div className="form-check" key={id}>
                          <input className="form-check-input bg-transparent border-secondary" type="checkbox"
                            checked={filters.activity === value} onChange={() => handleCheck("activity", value)} id={id} />
                          <label className="form-check-label" style={{ fontSize: "0.85rem", color: "#9fadbc" }} htmlFor={id}>{label}</label>
                        </div>
                      ))}
                    </div>
                  </div>

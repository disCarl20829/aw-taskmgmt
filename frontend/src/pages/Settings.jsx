...
const Settings = () => {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: bgColor, fontFamily: "Segoe UI, sans-serif", height: "100vh", overflow: "hidden" }}
    >
      <style>{`
        ...
        <section>
          ...
          <div className="d-flex align-items-center mt-3 mb-2 w-100 text-start px-2">
            ...
            <div className="d-flex flex-column gap-1 ps-4">
              ...
              <Link to="/settings" className="sidebar-btn-link active text-decoration-none">
                <i className="bi bi-gear me-2"></i> Settings
              </Link>
            </div>
        </section>
      ...
      <div style={{ flexGrow: 1, display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        {/* Scrollable content */}
        <div style={{ flexGrow: 1, overflowY: "auto", padding: "0 24px 24px 24px" }}>
          <div style={{ maxWidth: "800px" }}>
            ...
            <div className="w-100 position-relative">
              <SettingRow
                ...
                content={
                  <div>
                    ...
                    <span style={{ color: "#ff5c5c" }}>Private</span> – This Workspace is private.
                  </div>
                }
              />
              ...
              <SettingRow
                ...
                icon={<BriefcaseIcon />}
                hasChange
                onChangeClick={(e) => { e.stopPropagation(); setActiveOverlay("creation"); }}
                content={
                  <div style={{ color: "#b6c2cf" }}>
                    <p className="mb-1">{creationVisible} can create <PeopleIcon size={14} /> Workspace visible boards.</p>
                    <p className="mb-0">{creationPrivate} can create <LockIcon size={14} /> private boards.</p>
                  </div>
                }
              />
              {activeOverlay === "creation" && (
                <div ref={overlayRef} className="position-absolute shadow-lg p-3"
                  style={{ backgroundColor: "#282e33", width: "320px", right: "-40px", top: "-80px", zIndex: 100, borderRadius: "8px", border: "1px solid #444c54" }}>
                  <OverlayHeader title="Board creation restrictions" onClose={() => setActiveOverlay(null)} />
                  <div style={{ fontSize: "13px" }}>
                    <p className="fw-bold mb-2">Who can create <PeopleIcon size={12} /> Workspace visible boards?</p>
                    {["Any Workspace member", "Only Workspace admins", "Nobody"].map((opt) => (
                      <OptionItem key={opt} text={opt} isSelected={creationVisible === opt} onClick={() => setCreationVisible(opt)} blue={tradianBlue} />
                    ))}
                    <hr className="bg-secondary my-2" />
                    <p className="fw-bold mb-2">Who can create <LockIcon size={12} /> private boards?</p>
                    {["Any Workspace member", "Only Workspace admins", "Nobody"].map((opt) => (
                      <OptionItem key={opt} text={opt} isSelected={creationPrivate === opt} onClick={() => setCreationPrivate(opt)} blue={tradianBlue} />
                    ))}
                  </div>
                </div>
              )}
              ...
              <SettingRow
                ...
                icon={<BriefcaseIcon />}
                hasChange
                onChangeClick={(e) => { e.stopPropagation(); setActiveOverlay("deletion"); }}
                content={
                  <div style={{ color: "#b6c2cf" }}>
                    <p className="mb-1">{deletionVisible} can delete <PeopleIcon size={14} /> Workspace visible boards.</p>
                    <p className="mb-0">{deletionPrivate} can delete <LockIcon size={14} /> private boards.</p>
                  </div>
                }
              />
              {activeOverlay === "deletion" && (
                <div ref={overlayRef} className="position-absolute shadow-lg p-3"
                  style={{ backgroundColor: "#282e33", width: "320px", right: "-40px", top: "-80px", zIndex: 100, borderRadius: "8px", border: "1px solid #444c54" }}>
                  <OverlayHeader title="Board deletion restrictions" onClose={() => setActiveOverlay(null)} />
                  <div style={{ fontSize: "13px" }}>
                    <p className="fw-bold mb-2">Who can delete <PeopleIcon size={12} /> Workspace visible boards?</p>
                    {["Any Workspace member", "Only Workspace admins", "Nobody"].map((opt) => (
                      <OptionItem key={opt} text={opt} isSelected={deletionVisible === opt} onClick={() => setDeletionVisible(opt)} blue={tradianBlue} />
                    ))}
                    <hr className="bg-secondary my-2" />
                    <p className="fw-bold mb-2">Who can delete <LockIcon size={12} /> private boards?</p>
                    {["Any Workspace member", "Only Workspace admins", "Nobody"].map((opt) => (
                      <OptionItem key={opt} text={opt} isSelected={deletionPrivate === opt} onClick={() => setDeletionPrivate(opt)} blue={tradianBlue} />
                    ))}
                  </div>
                </div>
              )}
              ...
              <SettingRow
                ...
                icon={<BriefcaseIcon />}
                hasChange
                onChangeClick={(e) => { e.stopPropagation(); setActiveOverlay("guests"); }}
                content={
              ...
              {activeOverlay === "guests" && (
                <div ref={overlayRef} className="position-absolute shadow-lg p-3"
                  style={{ backgroundColor: "#282e33", width: "320px", right: "-40px", top: "75px", zIndex: 100, borderRadius: "8px", border: "1px solid #444c54" }}>
                  <OverlayHeader title="Inviting guests" onClose={() => setActiveOverlay(null)} />
                  <div style={{ fontSize: "13px" }}>
                    <p className="fw-bold mb-3">Who can Workspace boards be shared with?</p>
                    <OptionItem text="Anybody" isSelected={guestSelection === "Anybody"} onClick={() => setGuestSelection("Anybody")} blue={tradianBlue} subtext="Workspace boards can be shared with anybody." />
                    <OptionItem text="Only Workspace members" isSelected={guestSelection === "Only Workspace members"} onClick={() => setGuestSelection("Only Workspace members")} blue={tradianBlue} subtext="Workspace boards can only be shared with members of this Workspace." />
                  </div>
                </div>
              )}
            </div>
...
const OptionItem = ({ text, isSelected, onClick, blue, subtext }) => (
  <div className="mb-2 p-2 rounded option-hover" onClick={onClick} style={{ cursor: "pointer", color: isSelected ? blue : "white" }}>
    <div className="d-flex justify-content-between align-items-center">
      <span className="fw-bold">{text}</span>
      {isSelected && <CheckIcon color={blue} />}
    </div>
    {subtext && (
      <p className="mb-0" style={{ fontSize: "11px", opacity: 0.7, color: isSelected ? blue : "#b6c2cf" }}>{subtext}</p>
    )}
  </div>
);
...
const OverlayHeader = ({ title, onClose }) => (
  <div className="d-flex justify-content-between align-items-center mb-3 border-bottom border-secondary pb-2">
    <span className="fw-bold mx-auto" style={{ fontSize: "14px", color: "#b6c2cf" }}>{title}</span>
    <div onClick={onClose} style={{ cursor: "pointer", opacity: 0.7 }}><CloseIcon size={16} /></div>
  </div>
);
...
const SettingRow = ({ title, icon, content, hasChange, onChangeClick }) => (
  <div className="mb-4 w-100">
    <div className="border-bottom border-secondary pb-2 mb-2">
      <div className="fw-bold d-flex align-items-center" style={{ fontSize: "14px" }}>{title} {icon}</div>
    </div>
    <div className="d-flex justify-content-between align-items-start">
      <div style={{ fontSize: "14px", color: "#b6c2cf", flex: "1" }}>{content}</div>
      {hasChange && (
        <button onClick={onChangeClick} className="btn btn-sm px-3 ms-3"
          style={{ backgroundColor: "#2c333a", color: "#dee2e6", fontSize: "12px", border: "none", borderRadius: "3px" }}>
          Change
        </button>
      )}
    </div>
...
const BriefcaseIcon = () => (
  <svg width="14" height="14" fill="#9f8fef" viewBox="0 0 16 16" className="ms-2">

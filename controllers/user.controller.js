...
exports.searchByBoard = async (req, res) => {
    try {
        ...
        const [result] = await db.query('SELECT DISTINCT t1.user_id, t1.user_name, t1.user_email, t1.user_img_path, CASE WHEN t2.user_id IS NOT NULL THEN true WHEN t4.department_id IS NOT NULL THEN true ELSE false END AS isBoard FROM user AS t1 LEFT JOIN board_visibility AS t2 ON t1.user_id = t2.user_id AND t2.board_id = ? LEFT JOIN user_department AS t3 ON t1.user_id = t3.user_id LEFT JOIN board_visibility AS t4 ON t3.department_id = t4.department_id AND t4.board_id = ? WHERE t1.user_name LIKE ? OR t1.user_email LIKE ?',
            [board_id, board_id, search, search]
        );
...
exports.searchByCard = async (req, res) => {
    try {
        ...
        const [result] = await db.query('SELECT DISTINCT t1.user_id, t1.user_name, t1.user_email, t1.user_img_path, CASE WHEN t2.user_id IS NOT NULL THEN true WHEN t4.department_id IS NOT NULL THEN true ELSE false END AS isCardMember FROM user AS t1 LEFT JOIN card_member AS t2 ON t1.user_id = t2.user_id AND t2.card_id = ? LEFT JOIN user_department AS t3 ON t1.user_id = t3.user_id LEFT JOIN card_member AS t4 ON t3.department_id = t4.department_id AND t4.card_id = ? WHERE t1.user_name LIKE ? OR t1.user_email LIKE ?',
            [card_id, card_id, search, search]
        );
...
exports.addBoardMember = async (req, res) => {
    ...
    try {
        ...
        if ((!user_id && !department_id) || (user_id && department_id)) {
            await connection.rollback();

            return res.status(400).json({ message: "Must Provide Either user_id or department_id, not Both" });
        }

        if (user_id) {
            const [row] = await connection.query('SELECT 1 FROM board_visibility AS t1 LEFT JOIN user_department AS t2 ON t1.department_id = t2.department_id WHERE t1.board_id = ? AND (t1.user_id = ? OR t2.user_id = ?) LIMIT 1',
            ...
            await connection.query('INSERT INTO board_visibility (board_id, user_id, department_id) VALUES (?, ?, NULL)',
                [board_id, user_id]
            );

        } else {
            const [row] = await connection.query('SELECT 1 FROM board_visibility WHERE board_id = ? AND department_id = ? LIMIT 1',
                [board_id, department_id]
            );

            if (row.length !== 0) {
                await connection.rollback();

                return res.status(403).json({ message: "Department already has access to this board!" });
            }
        ...
        await connection.commit();
        res.json({ message: user_id ? "User successfully added to board!" : "Department successfully added to board!" });
    } catch (err) {
        if (connection) await connection.rollback();
        console.error(err);
...
exports.removeBoardMember = async (req, res) => {
    ...
    try {
        ...
        if (user_id) {
            [result] = await connection.query('DELETE FROM board_visibility WHERE board_id = ? AND user_id = ? AND department_id IS NULL',
                [board_id, user_id]
            );
        } else {
            [result] = await connection.query('DELETE FROM board_visibility WHERE board_id = ? AND department_id = ? AND user_id IS NULL',
                [board_id, department_id]
            );
        }
        ...
        if (result.affectedRows === 0) {
            await connection.rollback();
            return res.status(404).json({ message: user_id ? "User is not a member of this board" : "Department is not a member of this board" });
        }

        await connection.commit();
        res.json({ message: user_id ? "User was removed from board!" : "Department was removed from board!" });
    } catch (err) {
        if (connection) await connection.rollback();
        console.error(err);
...
exports.addCardMember = async (req, res) => {
    ...
    try {
        ...
        if (user_id) {
            ...
            const [assignedCard] = await connection.query(`SELECT 1 FROM card_member AS t1 LEFT JOIN user_department AS t2 ON t1.user_id = t2.user_id WHERE t1.card_id = ? AND (t1.user_id = ? OR t2.user_id = ?)`,
            ...
            await connection.query(
                'INSERT INTO card_member (card_id, user_id, department_id) VALUES (?, ?, NULL)',
                [card_id, user_id]
            );

        } else {
            const [row] = await connection.query(
                'SELECT 1 FROM board_visibility WHERE board_id = ? AND department_id = ? LIMIT 1',
                [board_id, department_id]
            );
            ...
            if (row.length === 0) {
                ...
                return res.status(403).json({ message: "Department is not a Board Member" });
            }

            const [assignedCard] = await connection.query(
                'SELECT 1 FROM card_member WHERE card_id = ? AND department_id = ? LIMIT 1',
                [card_id, department_id]
            );

            if (assignedCard.length > 0) {
                await connection.rollback();

                return res.status(403).json({ message: "This Department is Already a Member of this Card!" });
            }

            await connection.query('INSERT INTO card_member (card_id, user_id, department_id) VALUES (?, NULL, ?)',
                [card_id, department_id]
            );
        }

        await connection.commit();
        res.json({ message: user_id ? "User was added to card members" : "Department was added to card members" });
    } catch (err) {
        if (connection) await connection.rollback();
        console.error(err);
...
exports.removeCardMember = async (req, res) => {
    ...
    try {
        ...
        if (user_id) {
            [result] = await connection.query('DELETE FROM card_member WHERE card_id = ? AND user_id = ? AND department_id IS NULL',
                [card_id, user_id]
            );
        } else {
            [result] = await connection.query('DELETE FROM card_member WHERE card_id = ? AND department_id = ? AND user_id IS NULL',
                [card_id, department_id]
            );
        }
        ...
        if (result.affectedRows === 0) {
            ...
            return res.status(404).json({ message: user_id ? "User is not a member of this card" : "Department is not a member of this card" });
        }

        await connection.commit();

        return res.json({ message: user_id ? "User was Removed from Card" : "Department was Removed from this Card" })
    } catch (err) {
        if (connection) await connection.rollback();
        console.error(err);
